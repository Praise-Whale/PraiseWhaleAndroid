...
class CardBoxViewHolder (itemView : View) : androidx.recyclerview.widget.RecyclerView.ViewHolder(itemView){
    ...
    private val created_at : TextView = itemView.findViewById(R.id.tv_card_date)
    private val today_praise : TextView = itemView.findViewById(R.id.tv_card_praise)

    fun onBind(data: CollectionPraise){
        praiseName.text = ellipsis(data.praiseName, 5, "...")
        created_at.text = data.created_at.substring(5,6)+"월"+data.created_at.substring(8,9)+"일"
        today_praise.text = data.today_praise
    }
